...
const getBreedsImage = async (sortBy = "ASC") => {
    // tolong buat fetch API dengan parameter yang sudah didefinisikan diatas
    ...
    // 9b. return hasil fetch API dengan mengembalikan nilai json
    return response.json();
};

// 10. Buat fungsi fetchImage untuk melakukan pemanggilan fungsi getBreedsImage sesuai sortBy yang dikirim
const fetchImage = (sortBy = "ASC") => {
    // 10a. panggil fungsi getBreedsImage dengan parameter sortBy
    getBreedsImage(sortBy).then((response) => {
        // 10b. set item dari localStorage dengan key "petList" dan value dari response
        localStorage.setItem("petList", JSON.stringify(response));
        // 10c. render komponen dengan parameter response
